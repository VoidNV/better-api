...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	if !token.UnlimitedQuota && token.RemainQuota < quota {
		return fmt.Errorf("token quota is insufficient, remaining token quota: %s, required quota: %s", logger.FormatQuota(token.RemainQuota), logger.FormatQuota(quota))
	}
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	if !usePrice {
		logContent = fmt.Sprintf("Model magnification %.2f, completion magnification %.2f, audio magnification %.2f, audio completion magnification %.2f, grouping magnification %.2f",
			modelRatio, completionRatio.InexactFloat64(), audioRatio.InexactFloat64(), audioCompletionRatio.InexactFloat64(), groupRatio)
	} else {
		logContent = fmt.Sprintf("Model price %.2f, grouping magnification %.2f", modelPrice, groupRatio)
	}
	...
	if totalTokens == 0 {
		...
		quota = 0
		logContent += fmt.Sprintf("(probably upstream timeout)")
...
func checkAndSendSubscriptionQuotaNotify(relayInfo *relaycommon.RelayInfo) {
	gopool.Go(func() {
		...
		prompt := "Your subscription is about to be exhausted"
		topUpLink := fmt.Sprintf("%s/console/topup", system_setting.ServerAddress)
