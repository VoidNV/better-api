...
func DisableChannel(channelError types.ChannelError, reason string) {
	common.SysLog(fmt.Sprintf("An error occurred in channel \"%s\" (#%d) and is ready to be disabled. Reason: %s", channelError.ChannelName, channelError.ChannelId, reason))

	// 检查是否启用自动禁用功能
	if !channelError.AutoBan {
		common.SysLog(fmt.Sprintf("The automatic disabling function is not enabled for channel \"%s\" (#%d) and the disabling operation is skipped.", channelError.ChannelName, channelError.ChannelId))
		return
	}
