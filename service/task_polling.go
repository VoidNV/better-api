...
func sweepTimedOutTasks(ctx context.Context) {
	...
	legacyReason := "Task timeout (legacy tasks from the old system will not be refunded, please contact the administrator)"
	now := time.Now().Unix()
	timedOutCount := 0
...
func TaskPollingLoop() {
	for {
		...
		common.SysLog("Task progress polling starts")
		ctx := context.TODO()
...
func UpdateSunoTasks(ctx context.Context, taskChannelM map[int][]string, taskM map[string]*model.Task) error {
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("Channel #%d Update asynchronous task failed: %s", channelId, err.Error()))
		}
	}
	return nil
}
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	logger.LogInfo(ctx, fmt.Sprintf("Channel #%d Unfinished tasks are: %d", channelId, len(taskIds)))
	if len(taskIds) == 0 {
		return nil
	}
	...
	if err != nil {
		...
		err = model.TaskBulkUpdateByID(failedIDs, map[string]any{
			"fail_reason": fmt.Sprintf("Failed to obtain channel information, please contact the administrator, channel ID: %d", channelId),
			"status":      "FAILURE",
			"progress":    "100%",
		})
		if err != nil {
	...
	if !responseItems.IsSuccess() {
		common.SysLog(fmt.Sprintf("The unfinished tasks of channel #%d are: %d, the number of tasks successfully obtained: %s", channelId, len(taskIds), string(responseBody)))
		return err
	}
...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	...
	if bc := task.PrivateData.BillingContext; bc != nil && bc.PerCallBilling {
		logger.LogInfo(ctx, fmt.Sprintf("Task %s pay-per-view, skip settlement of difference", task.TaskID))
		return
	}
