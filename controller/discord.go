...
func getDiscordUserInfoByCode(code string) (*DiscordUser, error) {
	...
	if discordResponse.AccessToken == "" {
		common.SysError("Discord failed to obtain Token, please check the settings!")
		return nil, errors.New("Discord failed to obtain Token, please check the settings!")
	}
	...
	if res2.StatusCode != http.StatusOK {
		common.SysError("Discord Failed to obtain user information! Please check the settings!")
		return nil, errors.New("Discord Failed to obtain user information! Please check the settings!")
	}
	...
	if discordUser.UID == "" || discordUser.ID == "" {
		common.SysError("Discord Obtaining user information is empty! Please check the settings!")
		return nil, errors.New("Discord Obtaining user information is empty! Please check the settings!")
	}
...
func DiscordOAuth(c *gin.Context) {
	...
	if !system_setting.GetDiscordSettings().Enabled {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "The administrator has not enabled login and registration through Discord",
		})
		return
	}
	code := c.Query("code")
...
func DiscordBind(c *gin.Context) {
	...
	if model.IsDiscordIdAlreadyTaken(user.DiscordId) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "The Discord account has been bound",
		})
		return
	}
	session := sessions.Default(c)
