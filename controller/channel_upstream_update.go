...
func fetchChannelUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	...
	if channel.Type == constant.ChannelTypeGemini {
		...
		if apiErr != nil {
			return nil, fmt.Errorf("Failed to obtain channel key: %w", apiErr)
		}
...
func buildUpstreamModelUpdateTaskNotificationContent(
	...
	builder.WriteString(fmt.Sprintf(
		"Upstream model inspection summary: %d channels were detected, %d changes were found, %d were added, %d were deleted, %d were automatically synchronized, and %d failed.",
		checkedChannels,
		changedChannels,
		detectedAddModels,
	...
	if len(channelSummaries) > 0 {
		...
		builder.WriteString(fmt.Sprintf("Change channel details (display %d/%d):", displayCount, len(channelSummaries)))
		...
		if len(channelSummaries) > displayCount {
			builder.WriteString(fmt.Sprintf("- The remaining %d channels have been omitted", len(channelSummaries)-displayCount))
		}
	}
	...
	if len(normalizedAddModelSamples) > 0 {
		...
		builder.WriteString(fmt.Sprintf("New model example (showing %d/%d): %s",
			displayCount,
	...
	if len(normalizedRemoveModelSamples) > 0 {
		...
		builder.WriteString(fmt.Sprintf("Delete model example (showing %d/%d): %s",
			displayCount,
	...
	if failedChannels > 0 {
		...
		builder.WriteString(fmt.Sprintf(
			"Failed channel ID (showing %d/%d): %s",
			displayCount,
			failedChannels,
...
func runChannelUpstreamModelUpdateTaskOnce() {
	...
	if changedChannels > 0 || failedChannels > 0 {
		...
		service.NotifyUpstreamModelUpdateWatchers(
			"Upstream model inspection notice",
