...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		...
		if setting.CreemTestMode {
			logger.LogInfo(context.Background(), fmt.Sprintf("Creem webhook Signature verification has been skipped reason=test_mode signature=%q body=%q", signature, payload))
			return true
		}
		return false
	}
...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	if req.ProductId == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Please select product"})
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem product configuration parsing failed user_id=%d error=%q", c.GetInt("id"), err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Product configuration error"})
		return
	}
	...
	err = topUp.Insert()
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem Failed to create recharge order user_id=%d trade_no=%s product_id=%s error=%q", id, referenceId, selectedProduct.ProductId, err.Error()))
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem recharge order created successfully user_id=%d trade_no=%s product_id=%s product_name=%q quota=%d money=%.2f", id, referenceId, selectedProduct.ProductId, selectedProduct.Name, selectedProduct.Quota, selectedProduct.Price))
...
func RequestCreemPay(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem Payment request reading failed error=%q", err.Error()))
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook Signature verification successful path=%q client_ip=%s", c.Request.RequestURI, c.ClientIP()))
	...
	default:
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook Ignore event event_type=%s event_id=%s", webhookEvent.EventType, webhookEvent.Id))
...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	if event.Object.Order.Status != "paid" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem order status not paid, ignore processing request_id=%s order_id=%s order_status=%s", event.Object.RequestId, event.Object.Order.Id, event.Object.Order.Status))
		c.Status(http.StatusOK)
		return
	}
	...
	if event.Object.Order.Type != "onetime" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem This order type is not supported for the time being and will be ignored. request_id=%s creem_order_id=%s order_type=%s", referenceId, event.Object.Order.Id, event.Object.Order.Type))
		c.Status(http.StatusOK)
		return
	}
	...
	if topUp == nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem recharge order does not exist trade_no=%s creem_order_id=%s", referenceId, event.Object.Order.Id))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}

	if topUp.Status != common.TopUpStatusPending {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem recharge order status is not pending, ignore processing trade_no=%s status=%s creem_order_id=%s", referenceId, topUp.Status, event.Object.Order.Id))
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem recharge successful trade_no=%s creem_order_id=%s quota=%d money=%.2f client_ip=%s", referenceId, event.Object.Order.Id, topUp.Amount, topUp.Money, c.ClientIP()))
	c.Status(http.StatusOK)
}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Request to create HTTP failed: %v", err)
	}
