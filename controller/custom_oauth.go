...
func GetCustomOAuthProvider(c *gin.Context) {
	...
	if err != nil {
		common.ApiErrorMsg(c, "The OAuth provider was not found")
		return
	}
...
func FetchCustomOAuthDiscovery(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		common.ApiErrorMsg(c, "Invalid request parameter:"+err.Error())
		return
	}
	...
	if wellKnownURL == "" && issuerURL == "" {
		common.ApiErrorMsg(c, "Please fill in Discovery URL or Issuer URL first")
		return
	}
	...
	if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
		common.ApiErrorMsg(c, "Discovery URL is invalid, only http/https is supported")
		return
	}
	...
	if err != nil {
		common.ApiErrorMsg(c, "Create Discovery request failed:"+err.Error())
		return
	}
	...
	if err != nil {
		common.ApiErrorMsg(c, "Failed to obtain Discovery configuration:"+err.Error())
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		common.ApiErrorMsg(c, "Failed to obtain Discovery configuration:"+message)
		return
	}
...
func CreateCustomOAuthProvider(c *gin.Context) {
	...
	if model.IsSlugTaken(req.Slug, 0) {
		common.ApiErrorMsg(c, "The Slug is already used")
		return
	}

	// Check if slug conflicts with built-in providers
	if oauth.IsProviderRegistered(req.Slug) && !oauth.IsCustomProvider(req.Slug) {
		common.ApiErrorMsg(c, "This Slug conflicts with the built-in OAuth provider")
		return
	}
...
func DeleteCustomOAuthProvider(c *gin.Context) {
	...
	if count > 0 {
		common.ApiErrorMsg(c, "This OAuth provider also has user bindings that cannot be deleted. Please unbind all users first.")
		return
	}
...
func UnbindCustomOAuth(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "Unbinding successfully",
	})
}
