...
func UpdateMidjourneyTaskBulk() {
	...
	for {
		...
		logger.LogInfo(ctx, fmt.Sprintf("The number of unfinished tasks detected is: %v", len(tasks)))
		...
		for channelId, taskIds := range taskChannelM {
			...
			for _, responseItem := range responseItems {
				...
				if useTime > 3600000 && task.Progress != "100%" {
					responseItem.FailReason = "Upstream task timed out (more than 1 hour)"
				...
				if responseItem.VideoUrls != nil && len(responseItem.VideoUrls) > 0 {
					...
					if err != nil {
						logger.LogError(ctx, fmt.Sprintf("Serialization of VideoUrls failed: %v", err))
