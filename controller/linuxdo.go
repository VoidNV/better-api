...
func LinuxDoBind(c *gin.Context) {
	if !common.LinuxDOOAuthEnabled {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "The administrator has not enabled login and registration through Linux DO",
		})
		return
	}
	...
	if model.IsLinuxDOIdAlreadyTaken(user.LinuxDOId) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "The Linux DO account has been bound",
		})
		return
	}
	...
	if model.IsLinuxDOIdAlreadyTaken(user.LinuxDOId) {
		...
		if common.RegisterEnabled {
			if linuxdoUser.TrustLevel >= common.LinuxDOMinimumTrustLevel {
				...
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": "Linux DO The trust level does not reach the minimum trust level set by the administrator.",
				})
				return
			}
		} else {
