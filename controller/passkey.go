...
func PasskeyLoginFinish(c *gin.Context) {
	...
	handler := func(rawID, userHandle []byte) (webauthnlib.User, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("Passkey certificate not found: %w", err)
		}
...
func AdminResetPasskey(c *gin.Context) {
	...
	if _, err := model.GetPasskeyByUserID(user.Id); err != nil {
		if errors.Is(err, model.ErrPasskeyNotFound) {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "This user has not yet bound Passkey",
			})
			return
		}
		common.ApiError(c, err)
...
func requireSecureVerificationMethod(c *gin.Context, method string) bool {
	...
	if !ok || time.Now().Unix()-verifiedAt >= SecureVerificationTimeout {
		...
		session.Delete(secureVerificationMethodSessionKey)
		_ = session.Save()
		common.ApiErrorMsg(c, "Please complete the security verification first")
		return false
	}
