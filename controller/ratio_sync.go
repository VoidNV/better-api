...
const (
	...
	officialRatioPresetName     = "Official magnification preset"
...
func FetchUpstreamRatios(c *gin.Context) {
	...
	if len(req.Upstreams) > 0 {
		...
		if err != nil {
			...
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "Query channel failed"})
			return
		}
	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			if err := common.Unmarshal(body.Data, &pricingItems); err != nil {
				...
				ch <- upstreamResult{Name: uniqueName, Err: "Unable to parse upstream return data"}
				return
			}
