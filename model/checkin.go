...
func UserCheckin(userId int) (*Checkin, error) {
	...
	if !setting.Enabled {
		return nil, errors.New("Check-in function is not enabled")
	}
	...
	if hasChecked {
		return nil, errors.New("Signed in today")
	}
...
func userCheckinWithTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Create(checkin).Error; err != nil {
			return errors.New("Sign-in failed, please try again later")
		}

		// 步骤2: 在事务中增加用户额度
		if err := tx.Model(&User{}).Where("id = ?", userId).
			Update("quota", gorm.Expr("quota + ?", quotaAwarded)).Error; err != nil {
			return errors.New("Sign-in failed: Error in updating quota")
		}
...
func userCheckinWithoutTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	...
	if err := DB.Create(checkin).Error; err != nil {
		return nil, errors.New("Sign-in failed, please try again later")
	}
	...
	if err := IncreaseUserQuota(userId, quotaAwarded, true); err != nil {
		// 如果增加额度失败，需要回滚签到记录
		DB.Delete(checkin)
		return nil, errors.New("Sign-in failed: Error in updating quota")
	}
