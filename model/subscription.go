...
func CreateUserSubscriptionFromPlanTx(tx *gorm.DB, userId int, plan *SubscriptionPlan, source string) (*UserSubscription, error) {
	...
	if plan.MaxPurchasePerUser > 0 {
		...
		if count >= int64(plan.MaxPurchasePerUser) {
			return nil, errors.New("The purchase limit for this package has been reached")
		}
	}
...
func CompleteSubscriptionOrder(tradeNo string, providerPayload string, expectedPaymentProvider string, actualPaymentMethod string) error {
	...
	if logUserId > 0 {
		msg := fmt.Sprintf("Subscription purchased successfully, package: %s, payment amount: %.2f, payment method: %s", logPlanTitle, logMoney, logPaymentMethod)
...
func AdminInvalidateUserSubscription(userSubscriptionId int) (string, error) {
	...
	if downgradeGroup != "" {
		return fmt.Sprintf("User grouping will fall back to %s", downgradeGroup), nil
	}
	return "", nil
}
