...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			...
			logContent := fmt.Sprintf("Model fixed price %.2f, grouping magnification %.2f, operation %s", priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, constant.MjActionSwapFace)
...
func RelayMidjourneyTaskImageSeed(c *gin.Context) *dto.MidjourneyResponse {
	...
	if channel.Status != common.ChannelStatusEnabled {
		return service.MidjourneyErrorWrapper(constant.MjRequestError, "The channel to which this task belongs has been disabled")
	}
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if relayInfo.RelayMode == relayconstant.RelayModeMidjourneyImagine { //绘画任务，此类任务可重复
		...
		if originTask == nil {
			...
			c.Set("channel_id", originTask.ChannelId)
			c.Request.Header.Set("Authorization", fmt.Sprintf("Bearer %s", channel.Key))
			log.Printf("It is detected that this operation is amplification, transformation, and redrawing, and the original channel information is obtained: %s, %s", strconv.Itoa(originTask.ChannelId), channel.GetBaseURL())
		}
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			...
			tokenName := c.GetString("token_name")
			logContent := fmt.Sprintf("Model fixed price %.2f, grouping magnification %.2f, operation %s, ID %s", priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, midjRequest.Action, midjResponse.Result)
